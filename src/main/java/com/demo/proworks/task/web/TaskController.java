...
public class TaskController {
     ...
     * @throws Exception
     */
    @ElService(key = "TASK001Save")    
    @RequestMapping(value = "TASK001Save")
    @ElDescription(sub = "업무정보 저장 처리", desc = "업무정보를 저장 처리한다.")    
    public void saveTask(TaskListVo taskVoList) throws Exception {
            System.out.println("modified task : " + taskVoList.getTaskVoList().toString());
    	   
    	   int cnt = taskVoList.getTaskVoList().size();
    	   
    	   for(int i=0; i<cnt; i++){
    		   String rowStatus = taskVoList.getTaskVoList().get(i).getRowStatus();
    		   if(rowStatus.equals("C")) taskService.insertTask(taskVoList.getTaskVoList().get(i));
    		   else if(rowStatus.equals("D")) taskService.deleteTask(taskVoList.getTaskVoList().get(i));
    		   else if(rowStatus.equals("U")) taskService.updateTask(taskVoList.getTaskVoList().get(i));
    	   }
    }
    
    
    /**
	 * 업무 상태가 "대기"인 업무 목록을 조회합니다.
	 *
	 * @param taskVo 업무정보
	 * @return 목록조회 결과
	 * @throws Exception
	 */
	@ElService(key = "TASK001PendingList")
	@RequestMapping(value = "TASK001PendingList")
	@ElDescription(sub = "대기 상태인 업무 목록조회", desc = "대기 상태인 업무 목록조회")
	public TaskListVo selectPendingListTask(TaskVo taskVo) throws Exception {

		List<TaskVo> taskList = taskService.selectPendingListTask(taskVo);
