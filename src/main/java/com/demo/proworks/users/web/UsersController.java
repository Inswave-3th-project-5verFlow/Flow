...
public class UsersController {
	 ...
	 * @throws Exception
	 */
	@ElService(key = "SaveUser")
	@RequestMapping(value = "SaveUser")
	@ElDescription(sub = "사용자 정보를 다건 처리", desc = "사용자 정보를 다건 처리한다.")
	public void saveUsers(UsersListVo usersListVo) throws Exception {

		int cnt = usersListVo.getUsersVoList().size();

		for (int i = 0; i < cnt; i++) {
			UsersVo vo = usersListVo.getUsersVoList().get(i);
			String rs = vo.getRowStatus();
	...
	/**
	 * 특정 사용자의 비밀번호를 초기화 한다.
	 *
	 * @param usersVo 사용자정보
	 * @return 목록조회 결과
	 * @throws Exception
	 */
	@ElService(key = "ResetPwd")
	@RequestMapping(value = "ResetPwd")
	@ElDescription(sub = "비밀번호 초기화", desc = "특정 사용자 비밀번호를 초기화 한다.")
	public void resetPwd(UsersVo usersVo) throws Exception {
		usersService.resetPwd(usersVo);
	}
